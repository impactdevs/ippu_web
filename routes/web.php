...
Route::post('direct_attendence_certificate', [mEventsController::class, 'record_direct_attendence'])
    ->middleware('throttle:20,1')->name('direct.attendence');
...
Route::post("cpds/attendence/store", [CpdsController::class, 'storeAttendance'])->name('cpds.attendence.store');
    ...
    //events
    Route::post('events/attendence/update-email', [mEventsController::class, 'updateEmail'])->name('events.attendence.updateEmail');


    Route::post('/events/bulk-download', [mEventsController::class, 'downloadBulkCertificates'])->name('events.bulkDownload');
    Route::get('events/attendence-email/{id}/{user_id}', [mEventsController::class, 'emailCertificate']);
    Route::post('events/bulk-email', [mEventsController::class, 'sendBulkEmail'])->name('events.bulkEmail');
    Route::post('/events/bulk-download', [mEventsController::class, 'downloadBulkCertificates'])->name('events.bulkDownload');
