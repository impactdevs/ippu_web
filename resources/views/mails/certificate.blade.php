...
<head>
    ...
    <title>Certificate of Completion - {{ $event->code }}</title>
</head>
<body style="background-color: #f5f5f5; font-family: sans-serif; padding: 20px;">
    ...
    <div style="background-color: #0044cc; padding: 20px; text-align: center; color: white;">
        <img src="{{ url('/logo.png') }}" alt="IPPU Membership App Logo" width="200" height="100">
        <h1 style="margin-top: 10px;">Certificate of Completion</h1>
    </div>

    <div style="padding: 20px;">
        <p>Dear {{ $user->name }},</p>
        <p>Congratulations on successfully completing the <strong>{{ $event->topic }}</strong> course!</p>
        ...
        <ul>
            <li><strong>Course Code:</strong> {{ $event->code }}</li>
            <li><strong>Course Dates:</strong> {{ $formattedRange }}</li>
            <li><strong>CPD Hours:</strong> {{ $event->hours }} CPD HOURS</li>
        </ul>
    </div>
