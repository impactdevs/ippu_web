            ...
            <table class="table table-striped table-hover" id="members_table">
                ...
                <tbody>
                    <?php $__currentLoopData = $members; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $member): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr class="member-row <?php echo e($member->subscribed ? 'paid' : 'not-paid'); ?>">
                                ...
                                <div class="btn-group btn-group-sm">
                                    ...
                                    <ul class="dropdown-menu">
                                        <?php if($member?->latestMembership?->status == "Pending"): ?>
                                            <li><a class="dropdown-item" href="<?php echo e(url('admin/review_membership/'.$member->id)); ?>">Review Application</a></li>
                                        <?php endif; ?>
                                        <li>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        // Filter by payment status tabs
        $('#paymentTabs button').on('click', function () {
            var targetTab = $(this).attr('data-bs-target').replace('#', '');

            if (targetTab === 'paid') {
                table.rows().every(function() {
                    var row = this.node();
                    $(row).toggle($(row).hasClass('paid'));
                });
            } else if (targetTab === 'not-paid') {
                table.rows().every(function() {
                    var row = this.node();
                    $(row).toggle($(row).hasClass('not-paid'));
                });
            } else {
                table.rows().every(function() {
                    var row = this.node();
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Users/katendenicholas/Desktop/laravel/ippu_web/resources/views/admin/members/index.blade.php ENDPATH**/ ?>